...
hr {
    ...
    <script>
    window.currentLang = <?php echo json_encode(get_current_lang()); ?>;
    ...
    window.allGameRoles = <?php echo json_encode($all_game_roles); ?>;
    window.dbPhase = <?php echo json_encode($db['phase'] ?? 'setup'); ?>;
                        ...
                        if (!function_exists('get_role_status')) {
                            function get_role_status($role, $db, $gk_revealed) {
                                if ($role === 'Grave Keeper') {
                                    ...
                                    $gk_charges = $db['grave_keeper_charges'] ?? 2;
                                    if (!$has_gk || $gk_charges <= 0) return 'hidden';
                                    if ($is_gk_dead) return 'dead';
                                    return 'active';
                                }
                        ...
                        if (!function_exists('should_call_role')) {
                            function should_call_role($role, $db, $gk_revealed) {
                                ...
                                if ($role === 'Grave Keeper') {
                                    ...
                                    foreach ($db['players'] as $p) {
                                        if (($p['role'] ?? '') === $role && ($p['revealed'] ?? 'no') === 'no') {
                                            return true;
                                        }
                                    }
                                    return false;
                            ...
                            <?php if ($db['grave_keeper_revealed_roles'] ?? false): ?>
                                ...
                                <div class="mt-1 bg-indigo-950/40 border border-indigo-900/60 p-3 rounded-lg font-bold">
                                    ...
                                    if (!empty($revealed_roles)) {
                                        $role_badges = [];
                                        foreach ($revealed_roles as $k => $v) {
                                            $r_name = is_string($v) ? $v : ($v['role'] ?? 'Citizen');
                                            $role_badges[] = '<span class="inline-block bg-rose-950/80 text-rose-300 border border-rose-800/80 px-2.5 py-1 rounded text-xs font-bold shadow-sm">' . htmlspecialchars(get_role_label($r_name)) . '</span>';
                                        }
                                        echo '<div class="flex flex-wrap gap-1.5 mt-1">' . implode('', $role_badges) . '</div>';
                                    } else {
                    ...
                    <div class="bg-slate-900 border border-slate-800 p-6 rounded-xl space-y-4 shadow-lg">
                        ...
                        <?php if (empty($db['winner'])): ?>
                            <?php if ($db['roles_shared'] ?? false): ?>
                                ...
                                <?php else: ?>
                                <div class="bg-indigo-950/40 border border-indigo-900/60 p-3 rounded-lg text-center text-xs text-indigo-300 font-bold uppercase">
                                    <?php echo get_current_lang() === 'ku' ? 'تکایە دوگمەیا د بەشێ کریارێن شەڤێ دا بکاربینە' : (get_current_lang() === 'ar' ? 'يرجى استخدام الزر في قسم إجراءات الليل' : 'Please use the button in Night Actions section'); ?>
                                </div>
                                <?php endif; ?>
                            <?php else: ?>
