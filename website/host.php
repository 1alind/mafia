                ...
                <div class="bg-indigo-950/60 border-2 border-indigo-500/60 p-6 rounded-xl space-y-5 shadow-2xl">
                    ...
                    <p class="text-xs text-amber-300 bg-amber-950/40 border border-amber-900/60 p-3 rounded-lg">
                        ...
                        <?php if ($db['grave_keeper_revealed_roles'] ?? false): ?>
                            Because Grave Keeper agreed to reveal roles, you can <strong class="text-emerald-400">skip calling eliminated roles</strong> for this upcoming morning!
                        <?php else: ?>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        // Fixed condition: show Grave Keeper card during night if player exists and charges remain
                        $call_grave_keeper_tonight = ($has_grave_keeper && $gk_charges > 0);
                                ...
                                <div class="mt-2 bg-indigo-950/40 border border-indigo-900 p-3 rounded space-y-1">
                                    ...
                                    foreach ($db['players'] as $pl) {
                                        if ($pl['status'] === 'dead' || in_array($pl['name'], $db['delayed_departure'] ?? [])) {
                                            ...
                                            echo '<div class="text-xs text-rose-300 font-bold">•  <span class="text-white uppercase underline">' . htmlspecialchars($pl['role']) . '</span></div>';
                                        }
                                    }
                                    if (!$dead_roles_found) {
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-400 mt-0.5">
                                    ...
                                    Roles Revealed Status: <strong class="<?php echo ($db['grave_keeper_revealed_roles'] ?? false) ? 'text-emerald-400' : 'text-rose-400'; ?>">
                                        <?php echo ($db['grave_keeper_revealed_roles'] ?? false) ? 'Revealed for Today (Resets next night)' : 'Hidden'; ?>
                                    </strong>
                                </p>
                            </div>
                        </div>
